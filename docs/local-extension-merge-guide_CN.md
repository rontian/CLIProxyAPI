...
| OAuth 登录可靠性          | `/anthropic/callback`、`/codex/callback`、`/antigravity/callback`、`/xai/callback`、`/v0/management/get-auth-status`                 | `internal/api/server.go`、`internal/api/handlers/management/auth_files.go`、`internal/api/handlers/management/oauth_sessions.go`     | 回调页面成功不等于最终凭据保存成功；保留回调写入失败上报、Antigravity 换 token 超时和详细状态错误 |
